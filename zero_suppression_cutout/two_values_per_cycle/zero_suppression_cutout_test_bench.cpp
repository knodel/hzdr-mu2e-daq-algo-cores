...
void executeCore(int64_t *time, uint16_t *data, uint16_t *waveform, uint16_t &length) {
    ...
    in_stream_data cached_input;
    out_stream_data cached_output;

    //write data into input_stream (two values at a time)
    for (i = 0; i < WF_LENGTH; i++) {
        if ((i % 2) == 0) {
            cached_input.first_data = waveform[i];
            if (i == WF_LENGTH - 1) {
                cached_input.second_data = 0;
                input_stream << cached_input;
            }
        } else {
            cached_input.second_data = waveform[i];
            input_stream << cached_input;
        }
    }
    ...
    //read data from output_stream (two values at a time)
    for (i = 0; !output_stream.empty(); i += 2) {
        output_stream >> cached_output;
        data[i] = cached_output.first_data;
        time[i] = cached_output.first_time;
        data[i + 1] = cached_output.second_data;
        time[i + 1] = cached_output.second_time;
    }
    length = i;
}
